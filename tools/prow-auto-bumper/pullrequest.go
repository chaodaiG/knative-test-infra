...
func getExistingPR(title string) *github.PullRequest {
	return nil
}
...
func makeGitCommit(gi gitInfo, message string, dryrun bool) error {
	...
	if gi.user != "" && gi.email != "" {
		commitArgs = append(commitArgs, "--author", fmt.Sprintf("%s <%s>", gi.user, gi.email))
	}
	if err := run(
	...
	pushArgs := []string{"push", "-f", fmt.Sprintf("git@github.com:%s/%s.git", gi.user, gi.repo), fmt.Sprintf("HEAD:%s", gi.head)}
	if err := run(
...
func (gi *gitInfo) getHeadRef() string {
	return fmt.Sprintf("%s:%s", gi.user, gi.head)
}

func getOpenPR(gcw *GHClientWrapper, gi gitInfo, matchTitle string) (*github.PullRequest, error) {
...
func createOrUpdatePR(gcw *GHClientWrapper, pv *PRVersions, gi gitInfo, dryrun bool) error {
	title := makeCommitSummary(pv.getDominantVersions())
	body := generatePRBody()
	if err := makeGitCommit(gi, makeCommitSummary(pv.getDominantVersions()), dryrun); nil != err {
		return err
	}
	existPR, err := getOpenPR(gcw, gi, "Update prow from")
	if nil != err {
		return err
	}
	if nil != existPR {
		...
		return run(
			...
			func() error {
				_, err := gcw.UpdatePullRequest(gi.org, gi.repo, *existPR.Number, title, body)
				return err
			},
			dryrun)
	}
	return run(
		fmt.Sprintf("Creating PR, title: '%s', body: '%s'", *existPR.Number, title, body),
		func() error {
			_, err := gcw.CreatePullRequest(gi.org, gi.repo, gi.getHeadRef(), gi.base, title, body)
			return err
		},
		dryrun)
}
